	...
	for (float r = 20; r <= R * 0.25; r += 10) {
		...
		for (float phi_deg = 0; phi_deg < 360; phi_deg += phi_deg_step) {
			...
			for (float theta_deg = 0; theta_deg < 360; theta_deg += theta_deg_step) {
				...
				auto noise_value = ofNoise(glm::vec4(this->make_point(R, r, theta_deg, phi_deg) * 0.02, ofGetFrameNum() * 0.025));
				if (noise_value <= threshold_1 || noise_value >= threshold_2) { continue; }

				auto noise_1 = ofNoise(glm::vec4(this->make_point(R, r, theta_deg - theta_deg_step, phi_deg) * 0.02, ofGetFrameNum() * 0.025));
				auto noise_2 = ofNoise(glm::vec4(this->make_point(R, r, theta_deg, phi_deg + phi_deg_step) * 0.02, ofGetFrameNum() * 0.025));
				auto noise_3 = ofNoise(glm::vec4(this->make_point(R, r, theta_deg, phi_deg - phi_deg_step) * 0.02, ofGetFrameNum() * 0.025));
				auto noise_4 = ofNoise(glm::vec4(this->make_point(R, r, theta_deg + theta_deg_step, phi_deg) * 0.02, ofGetFrameNum() * 0.025));
